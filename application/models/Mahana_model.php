...
class Mahana_model extends CI_Model
{
    ...
    function send_new_message($sender_id, $recipients, $subject, $body, $priority)
    {
        ...
        $participants[] = array('thread_id' => $thread_id,'user_id' => $sender_id,'status' => 1);
        $statuses[]     = array('message_id' => $msg_id, 'user_id' => $sender_id,'status' => 1);

        if ( ! is_array($recipients))
        {
            $participants[] = array('thread_id' => $thread_id,'user_id' => $recipients, 'status' => 0);
            $statuses[]     = array('message_id' => $msg_id, 'user_id' => $recipients, 'status' => 0);
        }
        else
        {
            foreach ($recipients as $recipient)
            {
                $participants[] = array('thread_id' => $thread_id,'user_id' => $recipient, 'status' => 0);
                $statuses[]     = array('message_id' => $msg_id, 'user_id' => $recipient, 'status' => 0);
            }
        }
     ...
     */
    function reply_to_message($reply_msg_id, $sender_id, $body, $priority, $type='text')
    {
        $this->db->trans_start();
        ...
        // Add this message
        $msg_id = $this->_insert_message($thread_id, $sender_id, $body, $priority, $type);

        if ($recipients = $this->_get_thread_participants($thread_id, $sender_id))
        {
            $statuses[] = array('message_id' => $msg_id, 'user_id' => $sender_id,'status' => 1);
            $this->_update_participants(1,$thread_id,$sender_id);

            foreach ($recipients as $recipient)
            {
                $statuses[] = array('message_id' => $msg_id, 'user_id' => $recipient['user_id'], 'status' => 0);
                $this->_update_participants(0,$thread_id,$recipient['user_id']);
            }
    ...
    {
        $sql = 'SELECT m.*, s.status, t.subject,u.image,u.id as user_id, '.USER_TABLE_USERNAME .
    ...
    function update_thread_status($thread_id, $user_id, $status_id)
    {
        $this->db->where(array('thread_id' => $thread_id, 'user_id' => $user_id ));
        $this->db->update('msg_participants', array('status' => $status_id ));
    ...
    function threads_count($user_id)
    {
        $this->db->select("t.id");
     ...
     * @return  integer
     */
    private function _insert_message($thread_id, $sender_id, $body, $priority, $type)
    {
    ...
    private function _update_participants($status_id,$thread_id,$user_id)
    {
    ...
    public function get_all_user_threads($user_id,$limit=false)
    {
    ...
    public function delete_message($msg_id)
    {
        ...
        $this->db->delete('msg_messages');
        return true;
    }

    public function uploadFileConv()
    {
